...
andy="python3 ../bwt_based.py"
andy_c=./readmappers/bwt_based
...
declare -a preprocess_cmds=(
	true
	"$bwt -p $reference"
	"${andy} -p ${reference} ${reads}"
# 	"${andy_c} -p ${reference} ${reads}"
)
declare -a map_cmds=(
	"$exact -d $edits $reference $reads"
	"$bwt -d $edits $reference $reads"
	"${andy} -d ${edits} ${reference} ${reads}"
# 	"${andy_c} -d ${edits} ${reference} ${reads}"
)
